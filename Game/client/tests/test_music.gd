...
func _initialize() -> void:
	...
	if script:
		...
		_check(music.has_method("play_opening"), "Music 有 play_opening()")
		_check(music.has_method("play_world"), "Music 有 play_world()")
		_check(music.has_method("play_police_ambience"), "Music 有 play_police_ambience()")
		...
		_check(ResourceLoader.exists("res://audio/bgm/opening_slides.mp3"), "开场幻灯片音乐资源存在")
		_check(ResourceLoader.exists("res://audio/ambience/rain_world.ogg"), "雨声资源存在")
		_check(ResourceLoader.exists(music.POLICE_STATION), "警局环境音资源存在")
		music.free()
