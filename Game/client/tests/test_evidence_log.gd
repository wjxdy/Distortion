...
func _initialize() -> void:
	...
	if scene:
		...
		_check(not root.get_node("ToggleBtn/Dot").visible, "红点 Dot 默认隐藏")
		_check(not root.get_node("Panel/Detail").visible, "详情 Detail 默认隐藏")
		_check(not root.get_node("Toast").visible, "Toast 默认隐藏")
		root.free()
