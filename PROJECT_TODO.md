...
## 下一步
- [x] **【✅ 已部署到自有服务器 https://xuleii.cn/game/(评委用此HTTPS链接;Godot网页版必须安全上下文,IP明文HTTP会报Secure Context)】**（最终方案,详见 PROGRESS 同日条）。SSH root 原子部署到 `/opt/ai-demo-game/releases/20260620170450`+切 current 软链,纯静态、网页版直连 Moonshot。已验证 200+pck 字节一致+key 有效。**待用户**:① 浏览器实玩验收 LLM 回话;② Moonshot 设消费上限;③ demo 后轮换 key(已嵌公开包);④ 删 EdgeOne 测试项目 distortion/distortion-api/distortion-sizetest(控制台手删)。回滚:`ssh root@8.140.63.245 'ln -sfn /opt/ai-demo-game/releases/20260618005235 /opt/ai-demo-game/current'`。
- [ ] ~~【itch.io 部署】~~（未采用,改自有服务器;package_itch.sh 仍是出包工具）（方案已落地 6a58889，详见 PROGRESS 同日条；EdgeOne 方案已弃用——25MB限制+3h过期+ICP备案）。`build/distortion_web.zip`(97MB) 已生成。待用户：① **Moonshot 后台设消费上限**(key 会嵌入公开包)；② 上传 zip 到 **itch.io**(New project → Kind=HTML → 上传 zip → 勾 "This file will be played in the browser"；视口建议 1280×720 左右 → 拿到公开链接给评委)；③ 线上实测能玩 + 老头/终端 LLM 正常回话(Moonshot 已验证允许浏览器 CORS)；④ **demo 后轮换 key**；⑤ 删 EdgeOne 测试项目 `distortion`/`distortion-api`/`distortion-sizetest`(控制台手删,CLI 无删除命令)。重新打包随时跑 `bash scripts/package_itch.sh`。桌面/编辑器版不受影响。
