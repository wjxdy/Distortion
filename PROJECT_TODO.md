...
- [ ] 剧情去邪教版【客户端】落地，分步推进（步骤A已完成）：
  ...
  - [x] **手机可复用化 + 审讯室清理**：手机抽成 `phone.tscn`(world/审讯室共用);审讯室删「查档案」「回看记录」按钮;**档案已进手机**(点档案→显示+发 linxiulan 钥匙)。
  - [ ] **手机中枢第二刀剩余**：把 medical/neighbor/photo 也做成手机 app 内容(点→显示+发钥匙);**重点莫忘日志「滑坡」UI**(线索8,文案见桌面文档第五节/content.gd molog,第二层真相入口,需 molog 钥匙);线索可按顺序门控。做完后**退役终端室场景**(`terminal` 不再用,警局走廊终端室门改掉或留空)。全部色块占位+.tscn 节点。
