...
## 下一步
- [x] **【✅ 已部署到自有服务器 http://8.140.63.245/game/】**（最终方案,详见 PROGRESS 同日条）。SSH root 原子部署到 `/opt/ai-demo-game/releases/20260620170450`+切 current 软链,纯静态、网页版直连 Moonshot。已验证 200+pck 字节一致+key 有效。**待用户**:① 浏览器实玩验收 LLM 回话;② Moonshot 设消费上限;③ demo 后轮换 key(已嵌公开包);④ 删 EdgeOne 测试项目 distortion/distortion-api/distortion-sizetest(控制台手删)。回滚:`ssh root@8.140.63.245 'ln -sfn /opt/ai-demo-game/releases/20260618005235 /opt/ai-demo-game/current'`。
