...
- [ ] 主世界街道 F5 微调：背景已换**动态落雨纵深城市**(`world_rain_city.tres`,12帧)，主街 `world_street_road.png` 在上。待用户实机看：雨夜明暗/雨量是否合适、动画背景透出是否到位、墙带/门位/Player 起点手感；满意后无需再动。(原"等3张分层图+视差"方案已被这套动态背景取代，`world_far_city.png` 不再使用。)
