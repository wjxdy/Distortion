...
## 下一步
- [ ] **【进行中·分支 feat/finale-emergent-ending】终局重构 + 失踪妻子剧情改版**：设计稿已定+提交(62db481)，下一步 writing-plans 出实现计划 → subagent 落地 → 验证。范围见 spec `docs/superpowers/specs/2026-06-19-finale-emergent-ending-design.md`。本轮做:新剧情(复用场景只改文案)+老头/裁判双调用结局+4证据牌+修打断bug+删C结局。**延后第二阶段(留接口)**:通话记录"打通了"恐怖反转+当场打电话结局+AI合成语音。
